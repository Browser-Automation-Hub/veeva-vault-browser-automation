/**
 * auth.js — Authentication handler for Veeva Vault
 * Supports: SSO (Okta / Ping / Azure AD), MFA (Okta / Ping Identity MFA)
 *
...
const BASE_URL = process.env.VEEVA_VAULT_URL || 'https://your-vault.veevavault.com/ui/#login';
const USERNAME = process.env.VEEVA_VAULT_USERNAME;
const PASSWORD = process.env.VEEVA_VAULT_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set VEEVA_VAULT_USERNAME and VEEVA_VAULT_PASSWORD in .env`);
  }

  console.log('Logging in to Veeva Vault...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
