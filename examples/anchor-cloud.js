/**
 * anchor-cloud.js — AnchorBrowser cloud example for Veeva Vault
 *
...
const { login_vault, upload_document } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Veeva Vault automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_vault(page, {});
      log('Action 1 result:', data1);

      const data2 = await upload_document(page, {});
