# Contributing to Veeva Vault Browser Automation
...
- **Broken selectors**: Veeva Vault updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Veeva Vault instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.VEEVA_VAULT_URL}/path/to/section`);
...
When you find working selectors for Veeva Vault, document them:

```javascript
// Veeva Vault selectors (verified YYYY-MM-DD)
...
Veeva Vault updates its UI periodically. When selectors break:
